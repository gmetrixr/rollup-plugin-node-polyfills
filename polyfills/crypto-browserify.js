...
import string_decoder$2 from 'string_decoder';
...
var StringDecoder$2 = string_decoder$2.StringDecoder;
...
inherits_browser(Transform$3, _stream_duplex$1);
...
var readableBrowser_1$1 = readableBrowser$1.Stream;
var readableBrowser_2$1 = readableBrowser$1.Readable;
var readableBrowser_3$1 = readableBrowser$1.Writable;
var readableBrowser_4$1 = readableBrowser$1.Duplex;
var readableBrowser_5$1 = readableBrowser$1.Transform;
var readableBrowser_6$1 = readableBrowser$1.PassThrough;
var readableBrowser_7$1 = readableBrowser$1.finished;
var readableBrowser_8$1 = readableBrowser$1.pipeline;
...
function blind (priv) {
  var r = getr(priv);
  var blinder = r.toRed(bn$1.mont(priv.modulus)).redPow(new bn$1(priv.publicExponent)).fromRed();
...
function crt (msg, priv) {
  ...
  var len = priv.modulus.byteLength();
  var blinded = new bn$1(msg).mul(blinds.blinder).umod(priv.modulus);
  var c1 = blinded.toRed(bn$1.mont(priv.prime1));
  var c2 = blinded.toRed(bn$1.mont(priv.prime2));
...
for (key$3 in buffer$1) {
  if (!buffer$1.hasOwnProperty(key$3)) continue
  if (key$3 === 'SlowBuffer' || key$3 === 'Buffer') continue
  safer[key$3] = buffer$1[key$3];
}
...
PEMEncoder.prototype.encode = function encode(data, options) {
  const buf = der_1$1.prototype.encode.call(this, data);
...
function decrypt$2 (data, password) {
  ...
  var key = browser$4.pbkdf2Sync(password, salt, iters, keylen, 'sha1');
...
function getKey (x, q, hash, algo) {
  ...
  k = browser$3(algo, k).update(v).update(Buffer$H.from([0])).update(x).update(hbits).digest();
  v = browser$3(algo, k).update(v).digest();
  k = browser$3(algo, k).update(v).update(Buffer$H.from([1])).update(x).update(hbits).digest();
  v = browser$3(algo, k).update(v).digest();
  return { k: k, v: v }
}
...
function makeKey (q, kv, algo) {
  ...
  do {
    ...
    k = bits2int(t, q);
    kv.k = browser$3(algo, kv.k).update(kv.v).update(Buffer$H.from([0])).digest();
    kv.v = browser$3(algo, kv.k).update(kv.v).digest();
  } while (k.cmp(q) !== -1)
...
Object.keys(algorithms$2).forEach(function (key) {
  algorithms$2[key].id = Buffer$J.from(algorithms$2[key].id, 'hex');
...
var mgf = function (seed, len) {
  ...
  while (t.length < len) {
    c = i2ops(i++);
    t = Buffer$K.concat([t, browser$2('sha1').update(seed).update(c).digest()]);
  }
...
function oaep (key, msg) {
  ...
  var mLen = msg.length;
  var iHash = browser$2('sha1').update(Buffer$M.alloc(0)).digest();
...
function oaep$1 (key, msg) {
  var k = key.modulus.byteLength();
  var iHash = browser$2('sha1').update(Buffer$N.alloc(0)).digest();
  var hLen = iHash.length;
  if (msg[0] !== 0) {
    throw new Error('decryption error')
  }
